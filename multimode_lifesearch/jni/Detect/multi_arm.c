...
#define ROW 640
#define pre_row 128   //呼吸信号预处理数据道数，c'c
...
void breath_process(short result_data[]); //呼吸信号后处理函数
...
volatile unsigned int fflag = 0;			   //包数据计算
...
void breath_process(short result_data[]) //呼吸信号后处理过程
{
	...
	result_data[5] = (short)(breathmove); //上传呼吸信号判别结果
...
void z_ave_process(short result_data[]) //两道数据进行平均然后再进行处理
{
	int i = 0;
	if (ad_row_t < pre_num)
	{
...
void body_detect(short result_data[])
{
	...
	if (pflag == 1) //排除可能存在的虚警信号
	{
		...
		if (std_v < TH2)
			...
			bodymove = (signalpos + 27.0 * (pos_val + 1) / 256) * 15.0; //计算距离信息
	}
	result_data[0] = 0xDDAA;
	result_data[1] = 4;
	result_data[2] = pflag;
