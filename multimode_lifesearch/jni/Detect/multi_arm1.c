...
unsigned AD_ELEMENT_SIZE = 8192;//512
...
void detect(short ad_frame_data1[],short result_data1[], float);
...
float back_mov_data[8192/N_rowpoint-1]; //背景消除后回波
float ad_data_add[8192/N_rowpoint-1];//16位AD采样存放数组
float processdata_t[HARF_AD_SIZE][BACK_SIZE];

short send_ad_data[8192/N_rowpoint-1];  //向ARM发送的数组
...
void detect(short ad_frame_data1[],short result_data1[],float breath_th)
{
	int i,j/*,k*/;
	int last;
	for (i=0;i<6;i++)
...
{  
	...
	int starp=ceil((0.22/fs*N_columncompress)*ftnum);//频域能量起始点
	int	endp=floor((0.44/fs*N_columncompress)*ftnum);//频域能量终止点
...
int main()
{
	...
	// fw=fopen("E:\\数据\\搜救\\地下室\\20200616\\2600+400M\\处理文件夹\\ltefile106_p02.lte","rb");
