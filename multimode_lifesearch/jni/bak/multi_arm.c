...
#define  STATIC_LEN      2048/*512*/      //每道的元素个数
...
void detect(short ad_frame_data1[],short result_data1[]);
...
void breath_process(void);//呼吸信号后处理函数
...
float accumdata1[ACCU_LEN];              //每道数据加窗后结果
float processdata[STORE_ROW][REP_LEN]={0.};    //呼吸信号后处理数组数据
//unsigned short master_command[20];
short compressdata[STATIC_LEN-1]={0.};            //此压缩数据由host_dsp传送过来
float MMax[REP_LEN]={0.};
float enselect[ROW]={0.};                                  //最佳波形
float outdata1[ROW]={0.};                                //最佳波形滤波后输出结果?  
float Energy_breath[512/2]={0.};
short ad_frame_data1[AD_ELEMENT_SIZE];//此处存放原始数据
float compdata;
...
short preprocess_data1[AD_ELEMENT_SIZE];//此处存放预处理结果
float back_mov_data[AD_ELEMENT_SIZE/N_rowpoint-1]; //背景消除后回波
float ad_data_add[AD_ELEMENT_SIZE/N_rowpoint-1];//16位AD采样存放数组
float processdata_t[HARF_AD_SIZE][BACK_SIZE];

short send_ad_data[AD_ELEMENT_SIZE/N_rowpoint-1];  //向ARM发送的数组
...
///////////////////////////////////////////////////////
		...
		//	accumdata1[i]=accumdata1[i-1]+((float)compressdata[i+WIN-1]-(float)compressdata[i-1]);  /////滑动加窗
...
void breath_process(void)                            //呼吸信号后处理过程                   
{  
	...
	if ((signalpower/totalpower)>/*0.683*//*0.3*/1 && MMax[MMMaxposition]/MMean>=/*0.8*/1.5 ) 
	{ breathflag=1;
	...
	else {breathflag=0;breathmove=0;}
...
void pre_process(void)
{
	...
	for(i=0;i<SLAVE_SIZE;i++) 
	{
		...
		else  {ad_data_add[i]=(ad_data_add[i]*(BACK_SIZE-1)+compressdata[i])/BACK_SIZE;background_num1=301;}
...
void BandPass(int points,float *x,short *xout)
{
	...
	for(j=0;j<8;j++) {
		...
		if(Fdata1>=29000)  xout[j]=29000;
		else if (Fdata1<=-29000)  xout[j]=-29000;
		else xout[j]=(short)(Fdata1);
	...
	for(i=8;i<points;i++){
		...
		if(Fdata1>=29000)  xout[i]=29000;
		else if (Fdata1<=-29000)  xout[i]=-29000;
		else xout[i]=(short)(Fdata1);
	}
	// }
	free(MidVar);
}
...
{   
	...
	pre_num=2;
	slow_num=0;
	for(i=0;i<AD_ELEMENT_SIZE;i++) ad_frame_data1[i]=0;
	background_num1=0;
	for(i=0;i<SLAVE_SIZE;i++)
	{
		ad_data_add[i]=0.;
...
//	fp=fopen("processdata_t.txt","w");
//	fw=fopen("E:\\数据\\1_resample_p01.lte","rb");
//	//fw=fopen("E:\\数据\\搜救\\实验室呼吸\\LteFile-5_resample.lte","rb");
//	//fw=fopen("E:\\数据\\搜救\\五楼大厅穿北墙\\LteFile-9_resample.lte","rb");
...
//	{
//		fread(ad_frame_data1,sizeof(short),AD_ELEMENT_SIZE,fw);
//		detect(ad_frame_data1,result_data1);
//		for (i=1;i<6;i++)
//		{
