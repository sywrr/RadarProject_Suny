...
public class NetPacket extends Object {
	...
	// //定义数据包类型
	private int mPackFlag = 0xAAAABBBB; // String mPackFlag="netpacket";
										// //每一个数据包的标志
	// 数据包类型
	...
	public int receive(DataInputStream reader, boolean canTimeOut) {
		...
		try {
			...
			String msg = String.valueOf(mTransType) + " " + String.valueOf(mPackType) + " " + String.valueOf(mTransID) + " " + String.valueOf(mPackMaxLength) + " " + String.valueOf(mTotalLength) + " " + String.valueOf(mPackIndex) + " " + String.valueOf(mPackLength);
			// Log.e(TAG, msg);
	...
	public boolean isNeedACK() {
		// 如果是应答包，不需要应答
		if (mPackType == mAckPackType || mPackType == mWavePackType)
			return false;
		//
		return true;
	}
