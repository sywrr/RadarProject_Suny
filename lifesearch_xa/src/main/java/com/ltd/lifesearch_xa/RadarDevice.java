...
//		Log.e(TAG, "mDetingRangeL = " + String.valueOf(mDetingRangeL) + "mDetingRangeH = " + String.valueOf(mDetingRangeH) + "mDetectRange = " + String.valueOf(mDetectRange));
	}
...
//		Log.e(TAG, String.valueOf(low) + String.valueOf(high));
	...
	public void recvDatas(byte[] Bufs, int size)
	{
		if (mComBufWPos + size / 2 <= mComBufLen)
		{
			...
			{
				mComBuf[mComBufWPos + i] = (short)(Global.getUByte(Bufs[i * 2]) + (Global.getUByte(Bufs[i * 2 + 1]) << 8));
			}
			mComBufWPos += size / 2;
...
//		Log.e(TAG, "bufIndex = " + bufIndex + ",mNowWPos = " + mNowWPos[bufIndex]);
		...
		{
			mDibBufWPos = 0;
			((LifeSearchActivity)mContext).mDibView.changeDatasToDIB(mDibBuf, mComBufLen * 2);
			((LifeSearchActivity)mContext).mDibView.invalidate();
		}
		return rLen;
	}
	...
	public short[] readRadarDatas(int needLen)
	{
		...
		for (int i = 0; i < rLength / 2; i++)
		{
			...
			buf[i] = (short)(Global.getUByte(mDatasBufs[bufIndex][rPos + i * 2]) + (Global.getUByte(mDatasBufs[bufIndex][rPos + i * 2 + 1]) << 8));
		}
...
//		Log.e(TAG, "///" + mNowRPos[bufIndex] + "---" + mNowWPos[bufIndex] + "----" + retLen);
		return buf;
